...
## Wireshark: TCP <span style="color: limegreen">(5 баллов)<span>
...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - 128.119.245.12:80 (одинаковый порт для отправки и приёма)
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence number: 0 (RAW: 2692772477). Написано SYN в квадратных скобочках в информации о пакете и во флагах
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Sequence number: 0 (RAW: 165754935). 
   - Acknowledgment number: 1 (RAW: 2692772478). 
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Sequence number: 1 (RAW: 1379526708) 
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Sequence numbers: 1, 648, 2108, 3568, 5028, 6488. Ниже есть ACK пакеты с соответствующими ACK значениями. Они все были получены в одно время. Посчитаем среднее RTT:
   - Значения RTT в секундах - 0.112125 + 0.112100 + 0.112079 + 0.112032 + 0.112007 + 0.111983 (среднее: 0.112054)
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Среднее количество байт в секунду посчитаем так: раз уж все 6 ACK пакетов пришли в одно время 13.356424, то посчитаем сколько они занимают байт, и сколько в среднем доходили эти 6 пакетов (среднее RTT) (не сумма, потому что они пришли одновременно, а не друг за другом) и потом разделим (и ещё поделим на 2, так как RTT это соединение в 2 стороны): (701 + 1514 * 5) / 0.112054 / 2 = 36906 байт/с = 36.9 Кб/с. Не очень правильно оценивать пропускную способность по первым 6 пакетам, лучше было бы усреднить по всем пакетам этого TCP соединения, но это долго, и наверное тут подразумевается усреднить именно по первым 6.

### Работа с Time-Sequence-Graph (Stevens) <span style="color: limegreen">(2 балла)<span>
...
### А. Серверная часть <span style="color: limegreen">(2 балла)<span>
...
### Б. Клиентская часть <span style="color: limegreen">(2 балла)<span>
...
Если остановить сервер (Ctrl+C) во время пинга:

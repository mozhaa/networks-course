...
### <span style="color: limegreen">Задание А (3 балла)</span>
...
*Код сервиса я положил в папку service/*

### <span style="color: limegreen">Задание Б (3 балла)</span>
...
### <span style="color: limegreen">Задание В (4 балла)</span>
...
### <span style="color: limegreen">Задача 3 (2 балла)</span>
...
#### Решение
Вероятность того, что какой-то один конкретный пользователь в какой-то конкретный момент времени
использует канал равна $p = 0.2$. Тогда вероятность того, что хотя бы $12$ пользователей используют канал
в какой-то момент времени будет считаться как $p_{>=12} = 1 - p_{<12}$, а это, в свою очередь, 
значение функции распределения биномиального распределения с параметрами `n=12, p=0.2` в точке 11, и это можно посчитать так:
```python
import scipy.stats as ss
print(1 - ss.binom(60, 0.2).cdf(11))
```
Итоговая вероятность: $0.55$
